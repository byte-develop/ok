...
export default function Login() {
	const postData = async (id, login) => {
		await fetch('https://localhost:3001/user', {
			method: 'POST',
			headers: {
	...
	return (
		<div className='text-white flex flex-col items-center h-[100vw] w-[100vw]text-white mt-[25vh]'>
			<div className='flex flex-col  w-[17rem] '>
				...
				<p className='text-[35px] font-bold w-52 leading-[125%] mt-4'>
					Добро пожаловать!
				</p>
			...
			<form className='mt-5 w-[17rem]'>
				<input
					className='mb-6 h-14 w-[17rem] bg-transparent border-[1px] border-solid border-white border-opacity-25 rounded-md pl-[20px] text-[16px] text-white placeholder:text-[color: rgba(255, 255, 255, 0.35)]'
				...
				<Link to={'/home'}>
					<button
						onClick={() => postData('8888', logininput)}
						className='h-14 w-[17rem] bg-white text-black rounded-md'
					>
						Создать аккаунт
					</button>
				</Link>
